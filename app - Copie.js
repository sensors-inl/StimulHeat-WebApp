/**
 * Copyright (c) 2024 INL - INSA Lyon
 */
...
function reinitTempValues() {
    ...
    tRefElement.innerHTML = '--.- °C';
    tAmbElement.innerHTML = '--.- °C';
}
...
function setTempValues(tempValues) {
    ...
    tRefElement.innerHTML = (tempValues.getTemprefskin()*0.01).toFixed(2) + ' °C';
    tAmbElement.innerHTML = (tempValues.getTemprefamb()*0.01).toFixed(2) + ' °C';
}
...
function chartsAddMeasurement(results) {
    ...
    let tempSample = {x: time, y: results.getTemp() * 0.01}
    tempChart.data.datasets[0].data.push(tempSample);
